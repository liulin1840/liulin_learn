...
int need_clean_firewall = 1;

int wifidog_start_ok = 0;
...
append_x_restartargv(void)
{
    ...
    restartargv[i++] = m_strdup("-x");
    m_asprintf(&(restartargv[i++]), "%d", getpid());
}

#if 0     /*  Add by Liugj 2018-9-11*/
/* @internal
            ...
            while ((token1 = strsep(&running1, "|")) != NULL) {
                ...
                if (key && value && client) {
                    if (strcmp(command, "CLIENT") == 0) {
                        ...
                        if (strcmp(key, "ip") == 0) {
                            client->ip = m_strdup(value);
                        } else if (strcmp(key, "mac") == 0) {
                            client->mac = m_strdup(value);
                        } else if (strcmp(key, "token") == 0) {
                            client->token = m_strdup(value);
...
void
restore_clients_from_file(void)
{
    int count;

    /* load client-list */
    count = client_list_load_from_file(config_get_config()->clientlist_dump_file);
    remove(config_get_config()->clientlist_dump_file);    
    debug(LOG_INFO, "restoring client(count=%d)", count);
...
termination_handler(int s)
{
    ...
    debug(LOG_WARNING, "%s Exiting...", __func__);
    exit(s == 0 ? 1 : 0);
}
...
static void main_loop(void)
{
    ...
    log_to_file("Wifidog into main_loop");

    if ((config->debug_level >= 0) && (config->debug_level <= 7)) {
        debug_init2("wifidog", config->debug_level);
    } else {
        debug_init2("wifidog", LOG_WARNING);
    }

#if 0   /* Add by Liugj 2018-9-11. This action is done by wifidog_startup.sh. */ 
    /* Reset the firewall (if WiFiDog crashed) */
    fw_destroy();
#endif
    

    /* First start control thread, then startctl checking can reply in time. */
    result = pthread_create(&tid, NULL, (void *) thread_wdctl, (void *) m_strdup(config->wdctl_sock));
    if (result != 0) {
    ...
    /* init state is disconnect */
    super_uci_set_ext(" default.wifidog.state='disconnect'");
    ...
    if (!config->gw_address) {
        ...
        while (1) {
            if ((config->gw_address = get_iface_ip(config->gw_interface, NULL, 0)) != NULL) {
                break;
            }
            sleep(1);
        }
    ...
    if (!config->gw_id) {
        ...
        if ((config->gw_id = get_gw_id(NULL, 0)) == NULL) {
    ...
    while (1) {
        if ((webserver = httpdCreate(HTTP_ANY_ADDR, config->gw_port)) != NULL) {
            debug(LOG_DEBUG, "Create web server success\n");
            break;

        }
        sleep(2);
        debug(LOG_DEBUG, "Create web server, trying again");
    }
...
#endif
    httpdAddCContent(webserver, "/wifidog", "query", 0, NULL, http_callback_query);
    httpdAddCContent(webserver, "/wifidog", "acceptall", 0, NULL, http_callback_acceptall);
	httpdAddCContent(webserver, "/wifidog", "cnapass", 0, NULL, http_callback_cna_pass);

    httpdSetErrorFunction(webserver, 404, http_callback_404);

    wifi_intf_init();

    restore_clients_from_file();  /* add by Liugj 2018-9-9 */

    if (config->enable == 1) {
        /* When escape-mode enabled, fw-init after auth server online */
        if (!config->escape_mode_enable) {
            /* Then initialize it */
            if (!fw_init()) {
            ...
            debug(LOG_INFO, "Init firewall ok"); 
        }

        /* Added by David */
        result = pthread_create(&tid, NULL, (void *) wifidog_ac_thread, NULL);
        if (result != 0) {
    ...
    result = pthread_create(&tid, NULL, (void *) thread_tmp_client, NULL);
    if (result != 0) {
        debug(LOG_ERR, "FATAL: Failed to create a new thread (tmp_client) - exiting");
        termination_handler(0);
    }
    pthread_detach(tid);
	...
	if (config->thread_pool) {
		thread_pool_init(config->thread_num);
	}
    ...
    while (1) {
        ...
        if (webserver->lastError == -1) {
            ...
            params = m_malloc(2 * sizeof(void *));
            *params = webserver;
            *(params + 1) = r;

			if (config->thread_pool) {
				result= httpd_thread_wakeup((void*)params, 3); //3秒
			} else {
            ...
            if (result) {
                debug(LOG_CRIT, "FATAL: Failed to [create a new / wakeup] thread (httpd)  - drop request[%s]", r->clientAddr);
				m_free(params);
				httpdEndRequest(r);
                //termination_handler(0);
            }
...
gw_main(int argc, char **argv)
{
    ...
    /* Initialize url white/black list  */
    config_url_list();

    /* Initializes the linked list of connected clients */
    client_list_init();
    /* cleare br-fwdfast cache*/
//    br_fastfwd_mac_clear();


    /* Init the signals to catch chld/quit/etc */
    init_signals();
    log_to_file("Wifidog init done");

    if (restart_orig_pid) {
        /*
         * We were restarted and our parent is waiting for us to talk to it over the socket
         */
#if 0   /*add by Liugj 2018-9-8 as adding client_list_load_from_file(). */ 
